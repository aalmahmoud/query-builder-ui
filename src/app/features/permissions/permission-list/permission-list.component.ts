...
@Component({
  ...
  imports: [
    ...
    MatFormFieldModule, MatInputModule, MatProgressBarModule, FormsModule, RouterLink,
...
export class PermissionListComponent implements OnInit {
  ...
  loadData(): void {
    ...
    if (this.projecting) {
      ...
      this.permissionService.queryProjected(req, this.pageIndex, this.pageSize, sortParam).subscribe({
  ...
  onAggregate(req: AggregationRequest): void {
    ...
    this.permissionService.aggregate(req).subscribe({
  ...
  loadSavedQueries(): void {
    this.permissionService.getSavedQueries().subscribe({ next: (l) => (this.savedQueries = l), error: () => {} });
  }
  saveCurrentQuery(): void {
    const name = this.saveName.trim();
    if (!name) return;
    this.permissionService.createSavedQuery({ name, queryRequest: this.currentQuery }).subscribe({
  ...
  deleteSaved(sq: SavedQuery): void {
    this.permissionService.deleteSavedQuery(sq.id).subscribe({
